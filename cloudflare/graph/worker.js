...
// The Whisper security graph on Cloudflare Workers - 3.6B+ nodes of DNS/BGP/threat
// intelligence, queryable from the edge with plain fetch. Deploy: `wrangler deploy`.
//
...
//                   brand-impersonation scan). Set the key first:
//                   `wrangler secret put WHISPER_API_KEY`. A key on ANY call lifts the
//                   keyless rate limit.
//
...
export default {
  async fetch(request, env) {
    ...
    const apiKey = env.WHISPER_API_KEY; // optional: keyless works, a key lifts the rate limit
      ...
      if (target) {
        if (!apiKey) {
          return Response.json(
            { error: "catalog flows are keyed - set the WHISPER_API_KEY secret (the ?host= and ?variants= reads stay keyless)" },
            { status: 400 },
          );
        }
